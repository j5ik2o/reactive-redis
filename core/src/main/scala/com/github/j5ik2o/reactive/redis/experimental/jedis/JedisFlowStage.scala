          ...
          case Some(tc) =>
            run(t)(tc.ttl(t.key)) { result =>
              txState.append(ResponseF[lang.Long](result, { p =>
                TtlSucceeded(UUID.randomUUID(), t.id, p.get)
              }))
              TtlSuspended(UUID.randomUUID(), t.id)
            } { ex =>
              TtlFailed(UUID.randomUUID(), t.id, RedisIOException(Some(ex.getMessage), Some(ex)))
            }()
          case None =>
            run(t)(jedis.ttl(t.key)) { result =>
              TtlSucceeded(UUID.randomUUID(), t.id, result)
            } { ex =>
              TtlFailed(UUID.randomUUID(), t.id, RedisIOException(Some(ex.getMessage), Some(ex)))
            }()
        }
