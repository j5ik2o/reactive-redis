...
object TransactionOperations {
  ...
  object MultiRequest extends ResponseFactory {
  ...
  case class MultiRequest(id: UUID) extends SimpleRequest {
    ...
    override val responseFactory: ResponseFactory = MultiRequest
  }
  ...
  object ExecRequest extends TransactionResponseFactory {
    override def create(requestId: UUID, message: Reader[Char], responseFactories: Vector[ResponseFactory]): (Response, Reader[Char]) = {
      val result = parseResponse(message)
      result match {
        case (ArraySizeExpr(_), next) =>
          val result = responseFactories.foldLeft((next, Seq.empty[Response])){
            case ((n, seq), e) =>
              val (res, _n) = e.create(requestId, n)
              (_n, seq :+ res)
          }
